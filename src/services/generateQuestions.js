...
export const generateQuestions = async (transcriptText) => {
  ...
  3. Identify Core Content: Focus only on segments where core concepts or specific details related to the course material are discussed. Ignore introductory, filler, or unrelated content.
  4. Formulate Questions: For each categorized material, generate 10 questions that test understanding of the key concepts. Ensure the questions are clear and directly related to the material.
  5. Provide Options and Explanations: Include one correct answer and three plausible distractors for each question. Also, provide a concise explanation for why each option is either correct or incorrect.
  6. Code Formatting: Every instance of code, function names, or syntax elements in questions, options, or explanations, wrap the code in backtick so that it can be easily identified and styled as code in the frontend.
  7. Timestamp the Answer: Include the timestamp from the original data where the correct answer is derived.
  ...
  - "options": An array of four objects, each containing:
      ...
      - "explanation": The explanation for why the option is correct or incorrect.
  - "correctAnswer": The correct answer from the options.
  - "timestamp": The Ms in the original data where the correct answer is found.
  ...
  [
    {
      ...
      "options": [
        {
          "option": "func() is called",
          "explanation": "This is incorrect because in C, if the first operand of a logical AND (&&) is false, the second operand '''func()''' is not evaluated."
        },
        {
          "option": "'''func()''' is not called",
          "explanation": "This is correct because in C, the second operand is not evaluated when the first operand is false in a logical AND operation."
        },
        {
          "option": "x is evaluated first",
          "explanation": "This is partially correct, but doesn't fully answer the question. The question asks about the entire expression's result."
        },
        {
          "option": "The expression is true",
          "explanation": "This is incorrect. If x is false, the entire expression evaluates to false."
        }
      ],
      "correctAnswer": "'''func()''' is not called",
      "timestamp": 1853069
            ...
            .array(
              z.object({
                ...
                explanation: z
                  .string()
                  .describe(
                    "Explanation for why the option is correct or incorrect"
                  ),
              })
            )
            .length(4)
            .describe("Four options for the question with explanations"),
          correctAnswer: z
            .string()
            .describe("The correct answer among the options"),
          timestamp: z
            .number()
            .describe(
              "The timestamp from the original data where the correct answer is found"
            ),
        })
      ),
    }),
  ...
  const finalQuestion = convertQuestionFormat(generatedQuestions.questions);
...
const convertQuestionFormat = (questions) => {
  return questions.map((q) => {
    const options = q.options.map((opt) => opt.option);
    const explanations = q.options.map((opt) => opt.explanation);
